...
import myImg from "../../../assests/images/img16.jpg";
...
export default function Post({ postObject, allComments = false }: { postObject: PostType, allComments:boolean }) {
  ...
  return (
    <Card sx={{ width: "100%" }}>
      ...
      {postObject.comments.length > 0 && allComments == false  ? (
          ...
          <CardHeader
            avatar={
              <Avatar
                ...
                aria-label="recipe"
                onClick={() =>
                  handleNavigate(postObject.comments[0].commentCreator._id)
                }
              >
                <Image
                  src={handleImg(postObject.comments[0].commentCreator.photo)}
                  alt={postObject.comments[0].commentCreator.name}
                  width={50}
                  height={50}
                />
              </Avatar>
        ...
        postObject.comments?.map((comment:CommentsType) => <Box style={{ backgroundColor: "gray" }}>
        <CardHeader
          avatar={
            <Avatar
              sx={{ bgcolor: red[500], cursor: "pointer" }}
              aria-label="recipe"
              onClick={() =>
                handleNavigate(comment.commentCreator._id)
              }
            >
              <Image
      ...
      {/* ////////////////////////////////////////////// */}
      {postObject.comments.length > 1 && allComments == false? (
        <Typography
          style={{ cursor: "pointer" }}
          component="p"
          variant="h6"
          onClick={() => postDetails(postObject._id)}
        >
          view more comments..
        </Typography>
      ) : (
